const bcrypt = require('bcrypt');

module.exports = {
  async up(queryInterface) {
    await queryInterface.bulkInsert('Users', [{
      name: 'John',
      surname: 'Doe',
      email: 'John@mail.ru',
      password: await bcrypt.hash('adminadmin', 5),
      role: true,
      img: 'data:image/png;base64,',
      createdAt: new Date(),
      updatedAt: new Date(),
    }], {});

    await queryInterface.bulkInsert('Users', [{
      name: 'Dima',
      surname: 'Gachi',
      email: 'Dima@mail.ru',
      password: await bcrypt.hash('gachigachi', 5),
      role: false,
      img: 'data:image/png;base64,',
      createdAt: new Date(),
      updatedAt: new Date(),
    }], {});

    await queryInterface.bulkInsert('Users', [{
      name: 'Admin',
      surname: 'Admin',
      email: 'Admin@mail.ru',
      password: await bcrypt.hash('Qq12345/', 5),
      role: true,
      img: 'https://tr.rbxcdn.com/83d9da9e429a55be566b6b2c3f511e78/420/420/Image/Png',
      createdAt: new Date(),
      updatedAt: new Date(),
    }], {});
  },

  async down(queryInterface) {
    await queryInterface.bulkDelete('Users', null, {});
  },
};
